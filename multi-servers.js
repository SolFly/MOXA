...
socketMOXA1.on('connect', function() {
    ...
    for (let address = MOXA1USSStartAddress; address <= MOXA1USSEndAddress; address++) {
        clientMOXA1.readInputRegisters(0, 10, { unitId: address })
            .then(function(res) {
                dataMOXA1[address - 1] = res.response.data;
            }).catch(function(err) {
                console.log(err);
            });
    }
...
socketMOXA2.on('connect', function() {
    ...
    for (let address = MOXA2USSStartAddress; address <= MOXA2USSEndAddress; address++) {
        clientMOXA2.readInputRegisters(0, 10, { unitId: address })
            .then(function(res) {
                dataMOXA2[address - 1] = res.response.data;
            }).catch(function(err) {
                console.log(err);
            });
    }
